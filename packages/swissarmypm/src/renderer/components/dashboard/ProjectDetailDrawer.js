...
import { Drawer, Tabs, Descriptions, Tag, Progress, Button, Space, Typography, Empty, Spin } from 'antd';
...
export function ProjectDetailDrawer({ project, open, onClose, onEdit, onDelete, }) {
    ...
    const renderOverview = () => (_jsxs("div", { className: "space-y-6 animate-fade-in", children: [_jsxs("div", { className: "grid grid-cols-2 gap-4", children: [_jsxs("div", { className: "metric-card bg-gray-50 dark:bg-gray-800 p-4 rounded-lg hover:shadow-md transition-shadow", children: [_jsx(Text, { type: "secondary", children: "Progress" }), _jsxs("div", { className: "mt-2", children: [_jsx(Progress, { percent: metrics.progress, size: "small", strokeColor: {
                                            '0%': '#108ee9',
                                            '100%': '#87d068',
                                        }, className: "progress-bar-animated" }), _jsxs(Text, { className: "text-xs", children: [metrics.doneTasks, "/", metrics.totalTasks, " tasks completed"] })] })] }), _jsxs("div", { className: "metric-card bg-gray-50 dark:bg-gray-800 p-4 rounded-lg hover:shadow-md transition-shadow", children: [_jsx(Text, { type: "secondary", children: "Timeline" }), _jsx("div", { className: "mt-2", children: _jsxs(Text, { strong: true, children: [project.start_date ? new Date(project.start_date).toLocaleDateString() : 'Not set', " \u2192", ' ', project.end_date ? new Date(project.end_date).toLocaleDateString() : 'Not set'] }) })] })] }), project.description && (_jsxs("div", { children: [_jsx(Title, { level: 5, children: "Description" }), _jsx(Paragraph, { children: project.description })] })), project.tags && project.tags.length > 0 && (_jsxs("div", { children: [_jsx(Title, { level: 5, children: "Tags" }), _jsx(Space, { wrap: true, children: project.tags.map((tag) => (_jsx(Tag, { children: tag }, tag))) })] })), _jsxs("div", { children: [_jsx(Title, { level: 5, children: "Quick Stats" }), _jsxs(Descriptions, { column: 2, size: "small", children: [_jsx(Descriptions.Item, { label: "Status", children: _jsx(Tag, { color: getStatusColor(project.status), children: getStatusText(project.status) }) }), _jsx(Descriptions.Item, { label: "Owner", children: project.owner || 'Unassigned' }), _jsx(Descriptions.Item, { label: "Total Tasks", children: metrics.totalTasks }), _jsx(Descriptions.Item, { label: "Completed", children: metrics.doneTasks }), _jsx(Descriptions.Item, { label: "Blocked", children: metrics.blockerCount > 0 ? (_jsx(Tag, { color: "red", children: metrics.blockerCount })) : (_jsx("span", { className: "text-gray-500 dark:text-gray-400", children: "None" })) }), _jsx(Descriptions.Item, { label: "Milestones", children: metrics.milestoneCount }), _jsx(Descriptions.Item, { label: "Issues", children: metrics.issueCount }), _jsx(Descriptions.Item, { label: "Next Milestone", children: metrics.nextMilestone ? (_jsx("span", { children: metrics.nextMilestone.title })) : (_jsx("span", { className: "text-gray-500 dark:text-gray-400", children: "None" })) })] })] })] }));
    const renderWorkItems = () => (_jsx("div", { className: "space-y-4", children: workItems.length === 0 ? (_jsx(Empty, { description: "No work items yet", className: "animate-fade-in" })) : (_jsx("div", { className: "space-y-2", children: workItems.map((item, index) => (_jsxs("div", { className: "work-item-card border rounded-lg p-4 hover:bg-gray-50 dark:bg-gray-800 transition-all duration-200 hover:shadow-md animate-slide-in", style: { animationDelay: `${index * 50}ms` }, children: [_jsx("div", { className: "flex items-start justify-between", children: _jsxs("div", { className: "flex-1", children: [_jsxs("div", { className: "flex items-center gap-2 mb-1", children: [_jsx(Tag, { color: getStatusColor(item.status), className: "status-tag-animated", children: getStatusText(item.status) }), _jsx(Tag, { color: "blue", children: item.type }), _jsx(Text, { strong: true, children: item.title })] }), item.notes && (_jsx(Paragraph, { className: "mb-0 text-sm text-gray-500 dark:text-gray-400", children: item.notes })), (item.start_date || item.end_date) && (_jsxs(Text, { className: "text-xs text-gray-500 dark:text-gray-400 block mt-1", children: [item.start_date && `Start: ${new Date(item.start_date).toLocaleDateString()}`, item.start_date && item.end_date && ' | ', item.end_date && `End: ${new Date(item.end_date).toLocaleDateString()}`] }))] }) }), item.children && item.children.length > 0 && (_jsx("div", { className: "ml-4 mt-2 space-y-2", children: item.children.map((child, childIndex) => (_jsx("div", { className: "border-l-2 pl-3 py-1 hover:bg-gray-50 dark:bg-gray-800 transition-colors rounded animate-fade-in", style: { animationDelay: `${(index * 50) + (childIndex * 25) + 100}ms` }, children: _jsxs("div", { className: "flex items-center gap-2", children: [_jsx(Tag, { color: getStatusColor(child.status), children: getStatusText(child.status) }), _jsx(Text, { className: "text-sm", children: child.title })] }) }, child.id))) }))] }, item.id))) })) }));
