...
export function UpcomingMilestones({ milestones, loading }: UpcomingMilestonesProps) {
    ...
    return (
      <Card title={<span className="flex items-center gap-2 text-base"><Flag className="h-4 w-4" /> Upcoming Milestones</span>}>
        <div className="space-y-2">
          {[1, 2, 3].map((i) => (
            <div key={i} className="h-12 animate-pulse bg-gray-100 dark:bg-gray-700 rounded" />
          ))}
        </div>
      </Card>
    );
  ...
  return (
    ...
    >
      {milestones.length === 0 ? (
        <p className="text-center text-gray-500 dark:text-gray-400 py-4 text-sm">No upcoming milestones</p>
      ) : (
        <div className="space-y-1.5 max-h-[400px] overflow-y-auto">
          {milestones.map((milestone) => (
            <div
              key={milestone.id}
              className="flex items-center justify-between p-2 rounded border border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors cursor-pointer"
            >
              <div className="flex-1 min-w-0">
                <h4 className="font-medium truncate text-sm leading-tight">{milestone.name}</h4>
                <p className="text-xs text-gray-500 dark:text-gray-400 truncate mt-0.5">{milestone.projectName}</p>
              </div>
