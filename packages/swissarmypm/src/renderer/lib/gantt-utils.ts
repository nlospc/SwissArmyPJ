 ...
 */
export function formatDateForView(date: Date, viewMode: GanttViewMode): string {
...
export function generateDateHeaders(
  range: GanttDateRange,
  viewMode: GanttViewMode
): Array<{ date: Date; label: string; isToday: boolean; isWeekend: boolean }> {
  const headers: Array<{ date: Date; label: string; isToday: boolean; isWeekend: boolean }> = [];
  const totalDays = getTotalDays(range);
