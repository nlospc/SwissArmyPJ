...
export class ThemeValidator {
            ...
            if (element instanceof HTMLElement) {
                ...
                if (inlineStyle) {
                    problematicColors.forEach((problematicColor) => {
                        if (inlineStyle.includes(`background${problematicColor}`) ||
    ...
    checkMissingTransitions() {
        const interactiveElements = document.querySelectorAll('button, a, input, .ant-btn, .ant-card, .ant-input');
        interactiveElements.forEach((element) => {
            if (element instanceof HTMLElement) {
                const computedStyle = window.getComputedStyle(element);
                const transition = computedStyle.transition;
                // 检查是否有颜色相关的过渡
                const hasColorTransition = transition.includes('background') ||
                    transition.includes('color') ||
                    transition.includes('all') ||
                    transition.includes('ease');
                // 检查类名中是否包含主题相关的类
                const hasThemeClass = element.classList.contains('theme-transition') ||
